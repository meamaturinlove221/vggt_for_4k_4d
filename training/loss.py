...
class MultitaskLoss(torch.nn.Module):
    ...
    """
    def __init__(self, camera=None, depth=None, point=None, track=None, human_prior=None, **kwargs):
        super().__init__()
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        if self.human_prior is not None:
            human_prior_loss_dict = compute_human_prior_loss(predictions, batch, **self.human_prior)
            human_prior_loss = human_prior_loss_dict["loss_human_prior"] * self.human_prior.get("weight", 1.0)
...
def compute_human_prior_loss(predictions, batch, depth=None, point=None, **kwargs):
    ...
    Expected batch keys:
    - prior_mask / prior_masks: [B, S, H, W]
    - prior_depth / prior_depths: [B, S, H, W] or [B, S, H, W, 1]
    - prior_points: [B, S, H, W, 3]
    """
    total_prior_loss = _zero_loss_from_predictions(predictions)
    loss_dict = {"loss_human_prior": total_prior_loss}

    if depth is not None and "depth" in predictions:
        depth_loss_dict = compute_prior_depth_loss(predictions, batch, **depth)
        total_prior_loss = total_prior_loss + depth_loss_dict["loss_prior_depth"]
        loss_dict.update(depth_loss_dict)

    if point is not None and "world_points" in predictions:
...
def compute_prior_depth_loss(predictions, batch, gamma=1.0, alpha=0.0, gradient_loss_fn=None, valid_range=-1, **kwargs):
    ...
    if pred_depth_conf is None:
        pred_depth_conf = torch.ones(pred_depth.shape[:-1], device=pred_depth.device, dtype=pred_depth.dtype)

    prior_depth = _first_present(batch, "prior_depths", "prior_depth")
    prior_mask = _first_present(batch, "prior_mask", "prior_masks")
    dummy_loss = _zero_loss_from_predictions({"depth": pred_depth})

    if prior_depth is None or prior_mask is None:
        return {
            "loss_prior_depth_conf": dummy_loss,
            "loss_prior_depth_reg": dummy_loss,
            "loss_prior_depth_grad": dummy_loss,
            "loss_prior_depth": dummy_loss,
        }

    prior_depth = _canonicalize_depth_map(prior_depth)
    ...
    return {
        "loss_prior_depth_conf": loss_conf,
        "loss_prior_depth_reg": loss_reg,
        "loss_prior_depth_grad": loss_grad,
        "loss_prior_depth": total_loss,
    }
...
def compute_prior_point_loss(predictions, batch, gamma=1.0, alpha=0.0, gradient_loss_fn=None, valid_range=-1, **kwargs):
    ...
    prior_points = _first_present(batch, "prior_points")
    prior_mask = _first_present(batch, "prior_mask", "prior_masks")
    dummy_loss = _zero_loss_from_predictions({"world_points": pred_points})

    if prior_points is None or prior_mask is None:
        return {
            "loss_prior_point_conf": dummy_loss,
            "loss_prior_point_reg": dummy_loss,
            "loss_prior_point_grad": dummy_loss,
            "loss_prior_point": dummy_loss,
        }

    prior_points = _canonicalize_point_map(prior_points)
    prior_mask = _canonicalize_mask(prior_mask)
    ...
    total_loss = loss_conf + loss_reg + loss_grad
    return {
        "loss_prior_point_conf": loss_conf,
        "loss_prior_point_reg": loss_reg,
        "loss_prior_point_grad": loss_grad,
        "loss_prior_point": total_loss,
    }
...
def _canonicalize_mask(mask):
    if mask.ndim == 5 and mask.shape[-1] == 1:
        mask = mask[..., 0]
    elif mask.ndim == 5 and mask.shape[2] == 1:
        mask = mask[:, :, 0]
    elif mask.ndim != 4:
        raise ValueError(f"Expected prior mask with 4 or 5 dims, got {mask.shape}")
...
def _canonicalize_depth_map(depth):
    if depth.ndim == 4:
        return depth[..., None]
    if depth.ndim == 5 and depth.shape[-1] == 1:
        return depth
    if depth.ndim == 5 and depth.shape[2] == 1:
        return depth[:, :, 0].unsqueeze(-1)
    raise ValueError(f"Expected prior depth with shape [B, S, H, W] or [B, S, H, W, 1], got {depth.shape}")


def _canonicalize_point_map(points):
    if points.ndim != 5:
        raise ValueError(f"Expected prior points with 5 dims, got {points.shape}")
    if points.shape[-1] == 3:
        return points
    if points.shape[2] == 3:
...
def _resize_spatial_batch(tensor, target_hw, mode="bilinear"):
    ...
    if tensor.ndim == 4:
        resized = F.interpolate(
            tensor.reshape(batch_size * seq_len, 1, tensor.shape[2], tensor.shape[3]).float(),
            size=target_hw,
            mode=mode,
            align_corners=align_corners,
        )
        return resized.reshape(batch_size, seq_len, target_hw[0], target_hw[1])

    if tensor.ndim == 5:
        channels = tensor.shape[-1]
        resized = F.interpolate(
            tensor.permute(0, 1, 4, 2, 3).reshape(batch_size * seq_len, channels, tensor.shape[2], tensor.shape[3]).float(),
            size=target_hw,
            mode=mode,
            align_corners=align_corners,
        )
        return resized.reshape(batch_size, seq_len, channels, target_hw[0], target_hw[1]).permute(0, 1, 3, 4, 2).contiguous()
