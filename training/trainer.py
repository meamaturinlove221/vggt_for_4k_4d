...
from train_utils.normalization import (
    normalize_cam_points_with_scale,
    normalize_camera_extrinsics_and_points_batch,
    normalize_depths_with_scale,
    normalize_world_points_with_reference,
)
...
class Trainer:
    ...
    def save_checkpoint(self, epoch: int, checkpoint_names: Optional[List[str]] = None):
        ...
        """
        if not getattr(self.checkpoint_conf, "enabled", True):
            logging.info("Checkpoint saving disabled by config; skipping save at epoch %s", epoch)
            return
        ...
        if getattr(self.checkpoint_conf, "save_optimizer_state", True):
    ...
    def _process_batch(self, batch: Mapping):      
        ...
        normalized_extrinsics, normalized_cam_points, normalized_world_points, normalized_depths, normalization_aux = \
            normalize_camera_extrinsics_and_points_batch(
        ...
        if "prior_points" in batch and batch["prior_points"] is not None:
            batch["prior_points"] = normalize_world_points_with_reference(
                batch["prior_points"],
                reference_rotation=normalization_aux["reference_rotation"],
                reference_translation=normalization_aux["reference_translation"],
                avg_scale=normalization_aux["avg_scale"],
                name="prior_points",
            )

        if "prior_depths" in batch and batch["prior_depths"] is not None:
            batch["prior_depths"] = normalize_depths_with_scale(
                batch["prior_depths"],
                avg_scale=normalization_aux["avg_scale"],
                name="prior_depths",
            )

        if "prior_depth" in batch and batch["prior_depth"] is not None:
            batch["prior_depth"] = normalize_depths_with_scale(
                batch["prior_depth"],
                avg_scale=normalization_aux["avg_scale"],
                name="prior_depth",
            )

        if "prior_cam_points" in batch and batch["prior_cam_points"] is not None:
            batch["prior_cam_points"] = normalize_cam_points_with_scale(
                batch["prior_cam_points"],
                avg_scale=normalization_aux["avg_scale"],
                name="prior_cam_points",
            )
