...
def _stack_optional_tensor(value, dtype=None):
    if value is None:
        return None
    if torch.is_tensor(value):
        tensor = value
    else:
        array = np.stack(value) if isinstance(value, (list, tuple)) else np.asarray(value)
...
def _format_prior_maps(prior_maps: torch.Tensor) -> torch.Tensor:
    if prior_maps.ndim == 3:
        return prior_maps.unsqueeze(1)
    if prior_maps.ndim == 4 and prior_maps.shape[-1] <= 32 and prior_maps.shape[1] > 32 and prior_maps.shape[2] > 32:
        return prior_maps.permute(0, 3, 1, 2).contiguous()
    return prior_maps
...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        for key, dtype in optional_tensor_specs.items():
            ...
            tensor = _stack_optional_tensor(batch[key], dtype=dtype)
            if key == "prior_maps":
