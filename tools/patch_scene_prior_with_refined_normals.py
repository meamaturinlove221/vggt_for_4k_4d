...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", help="Checkpoint produced by train_detail_normal_refiner.py")
    ...
    parser.add_argument(
        ...
        default="coarse_valid",
    )
    parser.add_argument(
        "--summary-update",
        choices=("none", "mean_view_delta", "projected_token_sample"),
        default="none",
        help="Optional summary-token update mode for scene-level prior_summary_tokens.",
    )
    parser.add_argument(
        "--predictions-npz",
        default="",
        help="Predictions bundle with aligned intrinsics; required for --summary-update projected_token_sample.",
    )
...
def _lookup_indices(channel_names: np.ndarray, target_names: tuple[str, ...]) -> list[int]:
...
def _patch_prior_maps(
    *,
    ...
    summary_normal_indices: list[int] = []
    summary_position_indices: list[int] = []
    if "prior_summary_tokens" in prior_payload:
        prior_summary_tokens = np.array(prior_payload["prior_summary_tokens"], copy=True).astype(np.float32)
    if "prior_summary_channels" in prior_payload:
        summary_channels = np.array(prior_payload["prior_summary_channels"])
        if summary_update != "none":
            summary_normal_indices = _lookup_indices(summary_channels, SUMMARY_NORMAL_CHANNELS)
        if summary_update == "projected_token_sample":
            summary_position_indices = _lookup_indices(summary_channels, SUMMARY_POSITION_CHANNELS)
    ...
    for sample_idx in range(refined_normals.shape[0]):
        ...
        elif patch_mask_source == "teacher_mask":
            if "teacher_mask" not in dataset_payload:
                raise KeyError("--patch-mask-source teacher_mask requires teacher_mask in the dataset NPZ")
            patch_mask = np.asarray(dataset_payload["teacher_mask"][sample_idx], dtype=bool)
        else:
        ...
        if changed_pixels > 0:
            summary_delta = refined_roi[refined_mask].mean(axis=0) - coarse[sample_idx][patch_mask].mean(axis=0)
        summary_updated_tokens = 0
        if prior_summary_tokens is not None and summary_update == "mean_view_delta":
            ...
            prior_summary_tokens is not None
            and summary_update == "projected_token_sample"
        ...
        if sample_idx < max(1, int(visualize_count)):
            ...
            stitched_rgb = normal_to_rgb(
                _resize_float_hw(updated_patch, coarse_rgb.shape[0], coarse_rgb.shape[1], is_mask=False),
                _resize_float_hw(refined_mask.astype(np.float32), coarse_rgb.shape[0], coarse_rgb.shape[1], is_mask=True),
            )
    ...
    if prior_summary_tokens is not None:
        prior_payload["prior_summary_tokens"] = prior_summary_tokens.astype(np.float32)
...
def main() -> int:
    ...
    predictions_npz = Path(args.predictions_npz).expanduser().resolve() if args.predictions_npz.strip() else None
    intrinsics = None
    if str(args.summary_update) == "projected_token_sample":
        if predictions_npz is None:
            raise ValueError("--predictions-npz is required when --summary-update projected_token_sample")
        with np.load(predictions_npz, allow_pickle=False) as prediction_payload:
            if "intrinsic" not in prediction_payload:
                raise KeyError(f"{predictions_npz} is missing required key `intrinsic` for projected_token_sample")
            intrinsics = np.asarray(prediction_payload["intrinsic"], dtype=np.float32)

    dataset_payload = _load_dataset(dataset_npz)
    if str(args.normal_source) == "teacher":
        checkpoint_path = None
        refined_normals = np.asarray(dataset_payload["teacher_normal"], dtype=np.float32)
    else:
    ...
    summary = {
        ...
        "patch_mask_source": str(args.patch_mask_source),
        "summary_update": str(args.summary_update),
        "predictions_npz": None if predictions_npz is None else str(predictions_npz),
        ...
        "notes": [
            "Dense prior normal channels are always patched.",
            "Summary tokens are only updated when summary_update != none.",
