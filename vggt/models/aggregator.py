...
from vggt.models.human_prior import HumanPriorAdapter
    ...
    def forward(
        ...
        if prior_maps is not None:
            if self.human_prior_adapter is None:
                raise ValueError(
                    "prior_maps were provided, but the Aggregator was created with "
                    "human_prior_channels=0. Please enable the human prior adapter in the model config."
                )

            if prior_maps.ndim != 5:
                raise ValueError(f"Expected prior_maps with shape [B, S, C, H, W], got {prior_maps.shape}")
            if prior_maps.shape[:2] != (B, S):
                raise ValueError(
