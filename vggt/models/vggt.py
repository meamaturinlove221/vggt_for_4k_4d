    ...
    def forward(
        ...
        if prior_maps is not None:
            if prior_maps.ndim == 3:
                prior_maps = prior_maps.unsqueeze(0).unsqueeze(2)
            elif prior_maps.ndim == 4:
                prior_maps = prior_maps.unsqueeze(0)
            elif prior_maps.ndim != 5:
                raise ValueError(
                    f"Expected prior_maps to have 3, 4, or 5 dims, got {prior_maps.shape}"
                )

            if self.aggregator.human_prior_channels > 0:
                expected_channels = self.aggregator.human_prior_channels
